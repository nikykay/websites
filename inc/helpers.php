...
function get_share_link_url( $post_id, $network = 'facebook' ) {
	if ( empty( $post_id ) ) {
		return false;
	}
	...
	$thumb_url      = urlencode( wp_get_attachment_image_src( get_post_thumbnail_id( $post_id ), 'medium_large' )[0] );
	$share_url      = '';
 ...
 * @return string
 */
function get_address_url( $address ) {
 ...
 * @return string
 */
function section_class( $class, $block ) {
	if ( empty( $block ) ) {
		return '';
	}
	
	$class .= ' align' . $block['align'] ?? 'full ';
